...
@dataclass
class ReplicaTrainerConfig(TrainerConfig):
    _target: Type = field(default_factory=lambda: ReplicaTrainer)
...
class ReplicaTrainer(Trainer):
    ...
    def __init__(
        self, config: ReplicaTrainerConfig, local_rank: int = 0, world_size: int = 0
    ):
    ...
    def setup(self, test_mode: Literal["test", "val", "inference"] = "val"):
        ...
        pair is successfully streamed from online before allowing training to proceed.
        """
        ...
        status = False
        CONSOLE.print(
            f"[bold green] (NSonline) Waiting for for image streaming to begin ...."
        )
        ...
        if not status:
            ...
            CONSOLE.print(
                "[bold green] (NSonline) Dataloader is successfully streaming images!"
            )
