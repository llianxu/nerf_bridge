...
@dataclass
class DroidTrainerConfig(TrainerConfig):
    _target: Type = field(default_factory=lambda: DroidTrainer)
    msg_timeout: float = 600.0
    """ How long to wait (seconds) for sufficient images to be received before training. """
    num_msgs_to_start: int = 24
...
class DroidTrainer(Trainer):
    config: DroidTrainerConfig
    dataset: DroidDataset

    def __init__(
        self, config: DroidTrainerConfig, local_rank: int = 0, world_size: int = 0
    ):
    ...
    def setup(self, test_mode: Literal["test", "val", "inference"] = "val"):
        ...
        pair is successfully streamed from droid before allowing training to proceed.
        """
        # This gets called in the script that launches the training.
        # In this case ns_droid/droid_train.py
        ...
        status = False
        CONSOLE.print(
            f"[bold green] (NSDroid) Waiting for for image streaming to begin ...."
        )
        ...
        if not status:
            raise NameError(
                "DroidTrainer setup() timed out, check that topics are being published \
                 and that config.json correctly specifies their names."
            )
        else:
            CONSOLE.print(
                "[bold green] (NSDroid) Dataloader is successfully streaming images!"
            )
